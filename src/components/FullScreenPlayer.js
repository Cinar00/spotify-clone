...
function FullScreenPlayer({ toggle, state, controls, volumeIcon }) {
  ...
  return (
    <div
      ...
      <div
        className="absolute object-cover bg-no-repeat  bg-cover blur-sm opacity-30 inset-0 "
        style={{ backgroundImage: `url(${current.image}` }}
      />
	  <div className="absolute opacity-70 top-8 left-8 gap-x-3 text-white flex items-center">
	  ...
	  <div className="absolute left-8 bottom-36 flex items-center gap-x-5">
			...
			<div className="self-end">
				...
				<p className="text-sm font-medium opacity-60">{current.artist}</p>
			</div>
	  </div>
      <div onClick={e => {
				e.stopPropagation()
